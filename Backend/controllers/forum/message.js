...
exports.createMessage = (req, res, next) => {
	...
	const userId = decodedToken.userId;
	 ...
	 console.log("body", req.body._subject, req.body.text)
...
exports.getMessagesFromSubjectbypage = (req, res, next) => {
	...
	pool.query('SELECT * FROM messages LEFT JOIN person ON messages._user = person.user_uid where _subject = $1 ORDER BY datecreated DESC LIMIT 10 OFFSET $2', [req.params.id, page], (error, results) => {
		if (error){
			console.log('Erreur dans la création du message', error);
			res.status(400).json({ message: 'Erreur dans la création du message'})
		}
...
exports.updateMessage = (req, res, next) => {
	const date = new Date();
	pool.query('UPDATE messages SET text = $1, date = $2 WHERE message_uid = $3', (req.body.text, date, req.params.id), (error, results) => {
		if (error){
			throw error
		}
...
exports.deleteMessage = (req, res, next) => {
	...
	pool.query('DELETE FROM messages WHERE name = $1', (req.body.name), (error, results) => {
		if (error){
			throw error
		}
		res.status(200).json();
		...
		const path = req.file.path;
			...
			Jimp.read(path, (err, images) => {
				...
				else if (images.hasAlpha())
				{
					...
					image = `https://${req.get('host')}/api/images/x${req.file.filename}.webp`
				...
				images
					...
					image = `https://${req.get('host')}/api/images/x${req.file.filename}.jpeg`
					res.status(200).json({link: image}) 
				}
			})
